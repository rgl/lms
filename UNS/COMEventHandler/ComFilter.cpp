...
ComFilter::ComFilter(unsigned int maxEventID):eventsIDList_(maxEventID+1, 0, NULL), arrayMaxElement_(maxEventID)
{}

bool
ComFilter::addEvent(unsigned int eventID)
{
...
bool 
ComFilter::toSubscribe(const GMS_AlertIndication *alert) const
{
...
bool
ComFilter::defaultInitialization(std::shared_ptr<ComFilter> filter)
{
	if (
