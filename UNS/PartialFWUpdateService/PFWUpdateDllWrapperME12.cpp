...
struct PFWUpdateDllWrapperME12::FUNCTIONS
{
...
static std::wstring ME12_Dll_Name(L"FWUpdateLib_12.dll");


PFWUpdateDllWrapperME12::PFWUpdateDllWrapperME12(void)
{
	enum LoadFwDllState { LFDS_INITIAL, LFDS_FAILURE, LFDS_SUCCESS };

	static enum LoadFwDllState loadDllState = LFDS_INITIAL;

	if (loadDllState == LFDS_FAILURE)
	{
		throw std::exception("Dll state: LFDS_FAILURE");
	}
	loadDllState = LFDS_FAILURE;

	auto dllHandle = VerifyFile::SafeLoadDll(ME12_Dll_Name);
...
PFWUpdateDllWrapperME12::~PFWUpdateDllWrapperME12()
{
	delete functionsPtr;
}
...
uint32_t PFWUpdateDllWrapperME12::waitForFwInitDone() //this is W/A for ME10 and ME11
{
	return SUCCESS; 
}

uint32_t PFWUpdateDllWrapperME12::isPfwuRequired(bool& isLoclPfuRequired, bool& isWcodPfurequired, uint32_t& requiredLanguage)
{
	...
	try
	{
		...
		currentInstanceId = 0;
		status = functionsPtr->FwuPartitionInstancesDLL(FPT_PARTITION_NAME_WCOD, &currentInstanceId, &expectedInstanceId);
		if (status != SUCCESS)
		{
			goto End;
		}
...
uint32_t PFWUpdateDllWrapperME12::performPFWU(uint32_t partialID, const std::wstring& imagePath)
{
	uint32_t status = INTERNAL_ERROR;
