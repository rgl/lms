...
//
//  File:       IPS_IderSessionUsingPort.h
//
...
//
//----------------------------------------------------------------------------
#ifndef IPS_IDERSESSIONUSINGPORT_H
#define IPS_IDERSESSIONUSINGPORT_H 1
...
namespace Typed 
{
	// The IPS_IderSessionUsingPort is an association between the AMT_RedirectionService that manages the IDER session and the AMT_EthernetPortSettings instance that describes the I/F port through which the session is managed
	class CIMFRAMEWORK_API IPS_IderSessionUsingPort  : public IPS_SessionUsingPort
	{
	public:
		...
		IPS_IderSessionUsingPort(ICimWsmanClient *client)
		...
		//Destructor
		virtual ~IPS_IderSessionUsingPort(){}
		...
		// Class representing IPS_IderSessionUsingPort keys
		...
		static vector<shared_ptr<IPS_IderSessionUsingPort> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		IPS_IderSessionUsingPort(ICimWsmanClient *client, const string &class_name,
		...
		IPS_IderSessionUsingPort(const CimObject &object)
...
#endif // IPS_IDERSESSIONUSINGPORT_H
