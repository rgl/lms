...
//
//----------------------------------------------------------------------------
#ifndef CIM_COOLINGDEVICE_H
#define CIM_COOLINGDEVICE_H 1
...
namespace Typed 
{
	...
	class CIMFRAMEWORK_API CIM_CoolingDevice  : public CIM_LogicalDevice
	{
	public:
		...
		CIM_CoolingDevice(ICimWsmanClient *client)
		...
		// Class representing CIM_CoolingDevice keys
		...
		const bool ActiveCooling() const;

		// Optional, ActiveCooling is a Boolean that indicates that the Cooling Device provides active (as opposed to passive) cooling.
		void ActiveCooling(const bool value); 

		// Is true if the field ActiveCooling exists in the current object, otherwise is false.
		bool ActiveCoolingExists() const;
		...
		static vector<shared_ptr<CIM_CoolingDevice> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		CIM_CoolingDevice(ICimWsmanClient *client, const string &class_name,
		...
		CIM_CoolingDevice(const CimObject &object)
