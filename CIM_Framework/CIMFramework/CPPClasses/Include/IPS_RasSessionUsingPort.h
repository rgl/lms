...
//
//  File:       IPS_RasSessionUsingPort.h
//
...
//
//----------------------------------------------------------------------------
#ifndef IPS_RASSESSIONUSINGPORT_H
#define IPS_RASSESSIONUSINGPORT_H 1
...
namespace Typed 
{
	// The IPS_RasSessionUsingPort is an association between the CIM_RemoteAccessService that manages the CIRA session and the AMT_EthernetPortSettings instance that describes the I/F port through which the session is managed
	class CIMFRAMEWORK_API IPS_RasSessionUsingPort  : public IPS_SessionUsingPort
	{
	public:
		...
		IPS_RasSessionUsingPort(ICimWsmanClient *client)
		...
		//Destructor
		virtual ~IPS_RasSessionUsingPort(){}
		...
		// Class representing IPS_RasSessionUsingPort keys
		...
		static vector<shared_ptr<IPS_RasSessionUsingPort> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		IPS_RasSessionUsingPort(ICimWsmanClient *client, const string &class_name,
		...
		IPS_RasSessionUsingPort(const CimObject &object)
...
#endif // IPS_RASSESSIONUSINGPORT_H
