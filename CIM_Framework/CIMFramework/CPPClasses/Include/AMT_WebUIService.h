...
//
//----------------------------------------------------------------------------
#ifndef AMT_WEBUISERVICE_H
#define AMT_WEBUISERVICE_H 1
...
namespace Typed 
{
	// Represents the Web User-Interface service in AMT.
	class CIMFRAMEWORK_API AMT_WebUIService  : public CIM_Service
	{
	public:
		...
		AMT_WebUIService(ICimWsmanClient *client)
		...
		// Class representing AMT_WebUIService keys
		...
		static vector<shared_ptr<AMT_WebUIService> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		AMT_WebUIService(ICimWsmanClient *client, const string &class_name,
		...
		AMT_WebUIService(const CimObject &object)
			: CIM_Service(object)
		{
			if(_classMetaData.size() == 0)
			{
