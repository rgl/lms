...
//
//  File:       CIM_CredentialManagementService.h
//
...
//
//----------------------------------------------------------------------------
#ifndef CIM_CREDENTIALMANAGEMENTSERVICE_H
#define CIM_CREDENTIALMANAGEMENTSERVICE_H 1
...
namespace Typed 
{
	...
	class CIMFRAMEWORK_API CIM_CredentialManagementService  : public CIM_AuthenticationService
	{
	public:

		//Default constructor
		CIM_CredentialManagementService()
			: CIM_AuthenticationService(NULL, CLASS_NAME, CLASS_NS, CLASS_NS_PREFIX, CLASS_URI)
		{
			if(_classMetaData.size() == 0)
			{
				_classMetaData = CIM_AuthenticationService::GetMetaData();
			}
		}

		//constructor which receives WSMan client
		CIM_CredentialManagementService(ICimWsmanClient *client)
			: CIM_AuthenticationService(client, CLASS_NAME, CLASS_NS, CLASS_NS_PREFIX, CLASS_URI)
		{
			if(_classMetaData.size() == 0)
			{
				_classMetaData = CIM_AuthenticationService::GetMetaData();
		...
		//Destructor
		virtual ~CIM_CredentialManagementService(){}
		...
		// Class representing CIM_CredentialManagementService keys
		class CimKeys : public CIM_AuthenticationService::CimKeys
		{
		public:
		};
		...
		static vector<shared_ptr<CIM_CredentialManagementService> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		CIM_CredentialManagementService(ICimWsmanClient *client, const string &class_name,
			const string &class_ns, const string &prefix, const string &uri): CIM_AuthenticationService(client, class_name, class_ns, prefix, uri)
		{
			if(_classMetaData.size() == 0)
			{
				_classMetaData = CIM_AuthenticationService::GetMetaData();
		...
		CIM_CredentialManagementService(const CimObject &object)
			: CIM_AuthenticationService(object)
		{
			if(_classMetaData.size() == 0)
			{
				_classMetaData = CIM_AuthenticationService::GetMetaData();
		...
		{
			CIM_AuthenticationService::SetMetaData(childMetaData);
		}
...
#endif // CIM_CREDENTIALMANAGEMENTSERVICE_H
