...
//
//  File:       AMT_UserInitiatedConnectionService.h
//
...
//
//----------------------------------------------------------------------------
#ifndef AMT_USERINITIATEDCONNECTIONSERVICE_H
#define AMT_USERINITIATEDCONNECTIONSERVICE_H 1
...
namespace Typed 
{
	...
	class CIMFRAMEWORK_API AMT_UserInitiatedConnectionService  : public CIM_Service
	{
	public:
		...
		AMT_UserInitiatedConnectionService(ICimWsmanClient *client)
		...
		//Destructor
		virtual ~AMT_UserInitiatedConnectionService(){}
		...
		// Class representing AMT_UserInitiatedConnectionService keys
		...
		const string LastConnectionReason() const;

		// Optional, Represents the reason for the most recent user initiated connection.
		void LastConnectionReason(const string &value); 

		// Is true if the field LastConnectionReason exists in the current object, otherwise is false.
		bool LastConnectionReasonExists() const;

		// Remove LastConnectionReason field.
		void RemoveLastConnectionReason(); 
		...
		public:
			...
			// Reset: 11
			// DMTF Reserved: ..
			// All Interfaces disabled: 32768
			// BIOS Interface enabled: 32769
			// OS Interface enabled: 32770
			// BIOS and OS Interfaces enabled: 32771
		...
		static vector<shared_ptr<AMT_UserInitiatedConnectionService> > Enumerate(ICimWsmanClient *client, const CimKeys &keys = CimKeys()) ;
	...
	protected:
		...
		AMT_UserInitiatedConnectionService(ICimWsmanClient *client, const string &class_name,
		...
		AMT_UserInitiatedConnectionService(const CimObject &object)
			: CIM_Service(object)
		{
			if(_classMetaData.size() == 0)
			{
...
#endif // AMT_USERINITIATEDCONNECTIONSERVICE_H
